...
<!-- Badges de Métricas de Repositório -->
<br />
...
## 🌀 Meu Workflow DevOps & Mindset

Uso código e automação como ferramentas para garantir resiliência desde a infraestrutura até a entrega final:

```mermaid
graph LR
    A[Code / Git] --> B[CI/CD Pipelines]
    B --> C[Automation / PowerShell]
    C --> D[IDP / Platform Support]
    D --> E[Stability & Monitoring]
    style A fill:#7159c1,stroke:#fff,stroke-width:2px,color:#fff
    style B fill:#1e1e24,stroke:#7159c1,stroke-width:2px,color:#fff
    style C fill:#1e1e24,stroke:#7159c1,stroke-width:2px,color:#fff
    style D fill:#7159c1,stroke:#fff,stroke-width:2px,color:#fff
    style E fill:#111,stroke:#28a745,stroke-width:2px,color:#fff
